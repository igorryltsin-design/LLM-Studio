...
from adaptive_resource_trainer import (
    AdaptiveTrainingConfig as EvaluationConfig,
    AdaptiveTrainingError as EvaluationError,
    AdaptiveResourceTrainer,
    build_dataset,
)
...
class ChatRequest(BaseModel):
    model_path: str = Field(..., description="Путь до локальной модели ")
...
class EvaluationDatasetItemModel(BaseModel):
...
class EvaluationRequestModel(BaseModel):
    base_model_path: str = Field(..., description="Путь до базовой модели для тестирования")
    output_dir: Optional[str] = Field(
        default=None,
        description="Каталог для сохранения адаптера (по умолчанию Models/evaluation-<timestamp>)",
    )
    dataset: List[EvaluationDatasetItemModel] = Field(..., min_items=1)
...
class AgregatorRequestModel(BaseModel):
    ...
    endpoint: str = Field(
        "/export/csv",
        description="REST-эндпоинт Agregator для экспорта тестовых данных",
    )
...
class AutoEvaluationRequestModel(BaseModel):
    aggregator: AgregatorRequestModel
    base_model_path: str = Field(..., description="Путь до модели, для которой запускается тестирование")
    output_dir: Optional[str] = Field(
        default=None,
        description="Каталог для сохранения результатов (по умолчанию Models/evaluation-<timestamp>)",
    )
    config: EvaluationConfigModel
    include_previous_dataset: bool = Field(
        default=True,
        description="Если указан previous_evaluation_path, объединить старые примеры с новыми",
    )
    previous_evaluation_path: Optional[str] = Field(
        default=None,
    ...
    min_examples: int = Field(
        ...
        ge=1,
        le=100_000,
        description="Минимальное количество примеров для запуска тестирования",
    )
...
class EvaluationJobConfigResponse(BaseModel):
...
class EvaluationJobResponse(BaseModel):
    ...
    outputDir: str
    baseModelPath: str
    config: EvaluationJobConfigResponse
    events: Optional[List[EvaluationEventModel]] = None
...
_EVALUATION_MANAGER = AdaptiveResourceTrainer(root_dir=Path(__file__).parent)
...
def _fetch_agregator_examples(config: AgregatorRequestModel) -> List[Dict[str, Optional[str]]]:
    ...
    try:
        ...
        raise EvaluationError(f"Не удалось запросить тестовые примеры из Agregator ({url}): {exc}") from exc
    if response.status_code >= 400:
        raise EvaluationError(f"Agregator вернул статус {response.status_code}: {response.text[:200]}")
...
def _load_previous_dataset(previous_dir: Path) -> List[Dict[str, Optional[str]]]:
    ...
    if not dataset_path.exists():
        raise EvaluationError(f"Не найден сохранённый датасет тестов: {dataset_path}")
...
def _evaluation_config_from_request(request_config: EvaluationConfigModel) -> EvaluationConfig:
...
@app.post("/v1/evaluations", response_model=EvaluationJobResponse)
async def create_evaluation(request: EvaluationRequestModel):
    try:
        dataset = build_dataset(item.dict() for item in request.dataset)
        config = _evaluation_config_from_request(request.config)
        job = _EVALUATION_MANAGER.start_job(
...
@app.post("/v1/evaluations/from-agregator", response_model=EvaluationJobResponse)
async def create_evaluation_from_agregator(request: AutoEvaluationRequestModel):
    try:
        ...
        if request.previous_evaluation_path:
            previous_dir = Path(request.previous_evaluation_path).expanduser().resolve()
            if not previous_dir.exists() or not previous_dir.is_dir():
                raise EvaluationError(f"Каталог с предыдущими тестами не найден: {previous_dir}")
        ...
        if len(combined_examples) < request.min_examples:
            raise EvaluationError(
                "Недостаточно примеров для тестирования: %s (< %s)"
...
@app.get("/v1/evaluations", response_model=List[EvaluationJobResponse])
async def list_evaluations():
    return _EVALUATION_MANAGER.list_jobs()




@app.get("/v1/evaluations/{job_id}", response_model=EvaluationJobResponse)
async def get_evaluation(job_id: str):
    job = _EVALUATION_MANAGER.get_job(job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="Задача оценки качества не найдена")
    return job.to_dict()
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "registration_programs.metric_quality_evaluator:app",
        host="127.0.0.1",
        port=8012,
        reload=False,
    )
